---
layout: design-system-base.html
title: About Chobble Tickets
meta_title: About Chobble Tickets
meta_description: Chobble Tickets is built and run by Stefan Burke, a software developer in Prestwich, Manchester. Chobble is a UK community interest company registered with Companies House and the ICO.
permalink: /about/
eleventyNavigation:
  key: About
  order: 6
blocks:
  - type: hero
    class: gradient
    title: Who runs Chobble Tickets
    lead: >-
      Chobble Tickets is built and run by Stefan Burke, a software developer in
      Prestwich, Manchester. Chobble is a community interest company registered
      in England.

  - type: markdown
    content: |
      ## Stefan Burke

      Stefan Burke (known as Stef) has worked as a software developer for over
      20 years. He runs Chobble from Prestwich, Manchester. A photo and short
      profile is on [chobble.com/principles](https://chobble.com/principles).

      Before Chobble, Stef spent five years at
      [Bandcamp](https://bandcamp.com), the online music shop that pays out
      around $200 million a year to independent musicians. He worked first on
      the payments team and then on growth. Before Bandcamp, he was the lead
      developer at Bouncy Castle Network for ten years, where he wrote the
      booking software that took the company from one customer to over a
      thousand.

      Stef supports several Manchester community projects, including Blue Pits
      and Vegan Prestwich. The full CV is at
      [stefn.co.uk](https://stefn.co.uk).

  - type: markdown
    dark: true
    content: |
      ## Principles

      Chobble runs to six [stated principles](https://chobble.com/principles):

      - **Transparency.** No secrets are kept from customers. The tools they
        rely on are theirs to keep.
      - **Honesty.** Chobble declines work when another provider would suit the
        client better.
      - **Own, not rent.** Income comes from doing work, not from charging
        rent.
      - **Fair prices.** Flat prices set at the market rate for an experienced
        freelance programmer, with a 50% discount for charities, co-operatives,
        and artists.
      - **Freedom.** Customers have full control of their data and can move it
        to any provider.
      - **No harmful industries.** Chobble does not take work that exploits
        humans or animals.

      The full statement of each principle is at
      [chobble.com/principles](https://chobble.com/principles).

  - type: markdown
    content: |
      ## Chobble as a community interest company

      Chobble is registered as Chobble CIC, a
      [community interest company](https://www.gov.uk/government/publications/community-interest-companies-introduction)
      in England. The company number is 17050113.

      A community interest company is a UK legal structure for businesses with
      a social purpose. The structure includes an asset lock, which means
      profit and assets cannot be paid out to shareholders. If Chobble is wound
      up, the remaining assets transfer to another community-interest body.

      Chobble donates 10% of its income to the
      [Against Malaria Foundation](https://www.againstmalaria.com). The full
      social impact statement is at
      [chobble.com/social-impact](https://chobble.com/social-impact).

  - type: markdown
    dark: true
    content: |
      ## Registered office

      Chobble CIC's registered office is at 27 The Crescent, Prestwich,
      Manchester M25 1FL, United Kingdom.

      ## Company registration

      Community Interest Company number: 17050113.

      ## Data protection

      Chobble is registered with the
      [Information Commissioner's Office](https://ico.org.uk) (ICO), the UK
      data protection regulator. The registration covers Chobble's processing
      of personal data, including the data of event attendees on managed
      hosting.

      | Item | Value |
      |---|---|
      | Registration reference | [ZC097497](https://ico.org.uk/ESDWebPages/Entry/ZC097497) |
      | Date registered | 24 February 2026 |
      | Registration expires | 23 February 2027 |
      | Payment tier | Tier 1 |
      | Data controller | Chobble CIC |
      | Person responsible for data protection | Stefan Burke |

      For privacy queries, write to Stefan Burke at the registered office
      above.

  - type: cta
    title: Get started
    description: Sign up for managed hosting at £50/year, or download the source code from GitHub and self-host for free.
    button:
      text: Sign Up
      href: https://tix.chobble.com/ticket/register
      variant: primary
      size: lg
---

...
| `pages/features/registration-deadlines.md` | [x] Admin and public composite: `registration-deadline` | Fernbank Pottery's workshop starts at 10:00 on 16 August, closes at 18:00 the previous day and allows two places per buyer. Show the setting beside the public Booking has closed state. | Connects the organiser setting to the attendee result. |
| `pages/features/postcode-lookup.md` | [x] Attendee screenshot: `delivery-postcode-lookup` | Maya Patel books Willow Marquee Hire to `M25 1FL`. Show postcode search results, the selected editable address and a small run-sheet preview. | Connects faster address entry to accurate delivery operations. |
...
| `pages/features/images.md` | [x] Admin screenshot: `image-library` | Fernbank's library contains three workshop images with useful alt text. Show attachment counts, ordering controls and a protected `joining-instructions.pdf` attachment. | Demonstrates reuse, accessibility text and downloadable attendee files. |
...
| `pages/features/prepay-hosting.md` | Diagram: `prepay-term-lifecycle` | Oakfield pays £50 for a year and the term ends without renewal. Show the paid term, read-only mode opening with no new bookings but full read access and CSV and backup exports, a restart with no back-payment, and the 24-month deletion point. Beside it, a subscription line that stops at the first failed charge. | Shows that paying for hosting and keeping access to records are separate, so a missed renewal never removes data without notice. |
...
| `pages/perfect-for/sports-clubs.md` | [x] Admin screenshot: `sports-club-programme` | Riverside Rovers has a 2026 Home Fixtures group, £8 match tickets, £25 annual membership payment and Tuesday training with 20 places per date. | Shows tickets, one-off subscriptions and sessions coexisting without implying recurring direct debit. |
...
| `pages/perfect-for/festivals.md` | [x] Reuse: `required-child-listings-checkout.png` | Reuse the Willowbank Festival weekend-pass checkout with required General or Quiet Camping. | The existing screenshot already depicts the page's central festival workflow. |
| `pages/perfect-for/theatre-and-performing-arts.md` | [x] Admin screenshot: `theatre-front-of-house` | The Old Playhouse has 180 shared general-admission places across adult, child and concession tickets, with online, manual, complimentary and checked-in totals. | Shows shared house capacity and mixed sales while making the lack of allocated seating clear. |
...
| `guide-pages/cheapest-ticket-sales-system.md` | Comparison graphic: `annual-cost-crossover` | Plot the cost of a year of £20 tickets against the number sold, from 0 to 1,000. Draw one line for a flat annual fee plus processing and separate lines for the per-ticket rates of several reviewed providers, marking where each line crosses the flat one. | Shows that the cheapest provider depends on volume, and where the crossover falls. |
...
| `pages/compared-to/ticketmonkey.md` | Comparison graphic: `ticketmonkey-fee-crossover-and-free-events` | Harbourside Beer Festival sells 200 and 500 £12 tickets and also lists a 700-place free tasting session. Show Ticket Monkey's flat 25p per-ticket fee crossing Chobble's annual fee at about 200 tickets, and its 25p charge on free tickets beyond the 500-per-event cap, beside Chobble's flat fee and no-charge free events, with reserved seating and a box office as a separate Ticket Monkey advantage panel. | Shows both the break-even point and the free-event cap, without hiding Ticket Monkey's on-site tools. |
...
| `pages/compared-to/seaty.md` | Comparison graphic: `seaty-percentage-and-production-scope` | Penfold Players sells 400 £12 tickets across a four-night run and needs reserved seating, member quotas and a door kiosk. Show Seaty's 7.2% all-in online card fee and its 4.5% own-Stripe route beside Chobble's flat £50/year plus own-provider processing, with a timeline strip marking when each settles: Seaty holds the money until at least four days after the run, while the Stripe route and Chobble settle into the organiser's own payment account as each booking is made. Keep settlement and bank payout as separate marks. Show reserved seating, member ticket requests and balances, surveys, Gift Aid and the self-service kiosk as a separate Seaty advantage panel. | Shows where a higher percentage crosses the flat price sooner (near £1,150 of online sales at £15), and what a whole-production platform adds beyond the sale. |
| `pages/compared-to/promotix.md` | Comparison graphic: `promotix-who-pays-the-fee` | Marbury Sound Festival sells 600 £15 tickets. Show two price tags for the same ticket: £15 on Chobble and £16.72 on the PromoTix free plan, where the attendee pays 3.5% + £1.19 on top. Put the flat £50/year beside the £1,029 the fee raises across 600 tickets, and mark the crossover near 30 tickets. Add a panel for the PromoTix features Chobble has no answer to, being ambassador programmes, reserved seating, branded apps and ticketed streaming, and a separate panel showing the events marketplace both platforms treat differently. | Shows that a fee described as costing the organiser nothing still raises the price the attendee pays, and what the fee buys that a flat price does not. |
...
| `pages/compared-to/fleetbase.md` | Comparison graphic: `fleetbase-modules-versus-flat-fee` | Pepper Lane Supper Club sells 80 £18 dinner tickets a month and delivers half as meal boxes. Put Fleetbase's $29/month plus $5 per driver or vehicle, its Fleet-Ops, Storefront, Navigator and Ledger modules, route optimisation and live GPS in one column, beside Chobble's £50/year tickets plus hire plus run sheet, and label the features each platform does not have. | Shows where an open-source multi-module logistics platform overlaps a flat-fee ticketing tool, and where the per-driver pricing starts to add up. |
...
1. [x] Add `required-child-listings-checkout.png` to the Festivals page.
2. [x] Add `logistics-deliveries.png` to the Marquee Hire page.
...
10. [x] Theatre front-of-house dashboard

### Completed scenario files

- [x] `scripts/screenshots/oakfield-public-website.js`
- [x] `scripts/screenshots/duplicate-event.js`
- [x] `scripts/screenshots/delivery-postcode-lookup.js`
- [x] `scripts/screenshots/image-library.js`
- [x] `scripts/screenshots/mailgun-eu-settings.js`
- [x] `scripts/screenshots/named-admin-api-key.js`
- [x] `scripts/screenshots/registration-deadline.js`
- [x] `scripts/screenshots/grassroots-venue-door-list.js`
- [x] `scripts/screenshots/sports-club-programme.js`
- [x] `scripts/screenshots/taproom-repeating-quiz-night.js`
- [x] `scripts/screenshots/theatre-front-of-house.js`

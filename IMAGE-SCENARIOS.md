...
| `guide-pages/uk-made-ticket-platforms.md` | Diagram: `uk-registration-and-data-location` | Place each reviewed provider on two axes: country of company registration and country where attendee data is processed. Mark Chobble as UK-registered with Slovenian managed hosting and a UK self-hosting option. | Separates where a company is registered from where attendee records are stored. |
